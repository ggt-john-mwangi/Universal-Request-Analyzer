...
export function createTimeDimension(db) {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_time_period_15min ON dim_time(period_15min)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_time_period_1h ON dim_time(period_1h)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_time_period_4h ON dim_time(period_4h)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_time_period_1d ON dim_time(period_1d)`
  );
}
...
export function createDomainDimension(db) {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_domain_domain ON dim_domain(domain)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_domain_current ON dim_domain(is_current) WHERE is_current = 1`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_dim_domain_valid ON dim_domain(valid_from, valid_to)`
  );
}
...
export function createRequestFactTable(db) {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_requests_time ON fact_requests(time_key)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_requests_domain ON fact_requests(domain_key)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_requests_resource ON fact_requests(resource_type_key)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_requests_status ON fact_requests(status_code_key)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_requests_created ON fact_requests(created_at)`
  );
}
...
export function createPerformanceTrendFactTable(db) {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_trends_time_metric ON fact_performance_trends(time_key, metric_name)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_trends_domain ON fact_performance_trends(domain_key, metric_name)`
  );
}
...
export function createQualityMetricsFactTable(db) {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_quality_time ON fact_quality_metrics(time_key)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_fact_quality_domain ON fact_quality_metrics(domain_key)`
  );
}
...
export function getOrCreateDomainDimensionKey(db, domain, attributes = {}) {
  ...
  if (existing && existing.length > 0 && existing[0].values.length > 0) {
    ...
    const changed =
      (attributes.isThirdParty !== undefined &&
        attributes.isThirdParty !== !!isThirdParty) ||
      (attributes.category !== undefined && attributes.category !== category) ||
      (attributes.riskLevel !== undefined &&
