...
export class ConfigSchemaManager {
  ...
  async setAppSetting(key, value, options = {}) {
    try {
      ...
      this.eventBus?.publish("config:setting:changed", {
        key,
        value,
        category,
      });
  ...
  async getSettingsByCategory(category) {
    try {
      ...
      result[0].values.forEach(([key, value, dataType]) => {
        // Strip category prefix from key (e.g., "variables.list" -> "list")
        const keyWithoutCategory = key.startsWith(category + ".")
          ? key.substring(category.length + 1)
          : key;
        settings[keyWithoutCategory] = this.parseValue(value, dataType);
      });
  ...
  async setFeatureFlag(featureKey, enabled, options = {}) {
    try {
      ...
      this.eventBus?.publish("config:feature-flag:changed", {
        featureKey,
        enabled,
      });
      return true;
  ...
  async setUserPreference(userId, preferenceKey, preferenceValue) {
    try {
      ...
      this.eventBus?.publish("config:user-preference:changed", {
        userId,
        preferenceKey,
      });
      return true;
