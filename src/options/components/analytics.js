...
class Analytics {
  ...
  setupEventListeners() {
    ...
    if (trendCompareType) {
      trendCompareType.addEventListener("click", () =>
        this.loadTrendAnalysis()
      );
    }
  ...
  async loadDomainFilter() {
    try {
      ...
      if (
        ...
        console.log(
          `Loaded ${response.domains.length} domains for analytics filter`
        );
      }
    } catch (error) {
  ...
  displayAnomalies(anomalies) {
    ...
    const html = anomalies
      .map(
        (anomaly) => `
      <div class="anomaly-card severity-${anomaly.severity}">
        <div class="anomaly-header">
          <span class="anomaly-type">${this.getAnomalyTypeLabel(
            anomaly.type
          )}</span>
  ...
  displayTrends(trends) {
    ...
    this.updateTrendCard(
      "trendRequests",
      trends.current.totalRequests,
      trends.requestsChange
    );
    this.updateTrendCard(
      "trendDuration",
      `${trends.current.avgDuration}ms`,
      trends.durationChange
    );
    this.updateTrendCard(
      "trendErrors",
      trends.current.errors,
      trends.errorsChange
    );
    this.updateTrendCard(
      "trendBytes",
      this.formatBytes(trends.current.totalBytes),
      trends.bytesChange
    );
  }
  ...
  updateTrendCard(id, value, change) {
    ...
    if (changeEl) {
      ...
      changeEl.className = `trend-change ${
        changeNum > 0 ? "positive" : changeNum < 0 ? "negative" : ""
      }`;
    }
  }
  ...
  renderHeatmap(heatmap) {
    ...
    heatmap.hours.forEach((hour, idx) => {
      if (idx % 3 === 0) {
        // Show every 3rd hour to avoid clutter
