import { generateRequestId } from "../../background/utils/message-handler";
...
export default function renderAnalyticsSection() {
  ...
  let activeAnalysisChart = analysisCharts[0];
  ...
  analysisCharts.forEach((chart) => {
    ...
    btn.className =
      "chart-tab-btn" + (chart.id === activeAnalysisChart ? " active" : "");
    btn.onclick = () => {
      if (
        !visualizationCheckboxes[chart.id] ||
        !visualizationCheckboxes[chart.id].checked
      )
        return; // Only allow if enabled
  ...
  analysisCharts.forEach((chart) => {
    ...
    checkbox.addEventListener("change", () => {
      ...
      if (!checkbox.checked && activeAnalysisChart === chart.id) {
        const next = analysisCharts.find(
          (c) => visualizationCheckboxes[c.id].checked
        );
        if (next) {
  ...
  perfCharts.forEach((chart) => {
    ...
    btn.textContent = chart.label;
    btn.className =
      "chart-tab-btn" + (chart.id === activePerfChart ? " active" : "");
    btn.onclick = () => {
  ...
  chrome.runtime.sendMessage({
    action: "getDistinctDomains",
    requestId: domainControlReqId,
  });
  ...
  if (
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (
        message &&
        message.requestId &&
        pendingAnalyticsRequests[message.requestId]
      ) {
  ...
  async function loadAnalytics() {
    if (typeof Chart === "undefined") {
      console.error(
        "Chart.js is not loaded. Please ensure lib/chart.min.js is included in options.html."
      );
      summary.innerHTML =
        '<div class="error">Chart.js not loaded. Charts cannot be displayed.</div>';
      return;
    }
    ...
    function renderAnalytics() {
      ...
      const types = Array.from(
        controls.querySelectorAll("#analytics-types input:checked")
      ).map((i) => {
      ...
      console.log(
        "[Analytics] Requesting getFilteredStats",
        filters,
        requestId
      );
      pendingAnalyticsRequests[requestId] = (stats) => {
        ...
        if (!stats || stats.error) {
          summary.innerHTML = `<div class='error'>${
            stats?.error || "No analytics data."
          }</div>`;
          return;
        }
        // Feature/ACL gating
        const canViewPerf =
          (features.performanceTimings ?? true) &&
          (config.capture?.includeTiming ?? true);
        ...
        const showTimeDist =
          controls.querySelector("#toggle-time-dist").checked;
        const showSizeDist =
          controls.querySelector("#toggle-size-dist").checked;
        ...
        responseDiv.style.display =
          canViewApi && showResponse ? "block" : "none";
        statusDiv.style.display = canViewApi && showStatus ? "block" : "none";
        typesDiv.style.display = canViewApi && showTypes ? "block" : "none";
        timeDistDiv.style.display =
          canViewApi && showTimeDist ? "block" : "none";
        sizeDistDiv.style.display =
          canViewApi && showSizeDist ? "block" : "none";
        perfDiv.style.display = canViewPerf && showPerf ? "block" : "none";
        // --- Summary ---
        summary.innerHTML = `
          <div><strong>Total:</strong> ${stats.requestCount}</div>
          <div><strong>Avg Time:</strong> ${
            stats.avgResponseTime ? stats.avgResponseTime.toFixed(2) : 0
          } ms</div>
        ...
        if (api && stats.apiResponseTimes && stats.apiResponseTimes[api]) {
          ...
          responseChart = new Chart(rtCanvas.getContext("2d"), {
            ...
            options: {
              plugins: {
                title: { display: true, text: `Response Time for ${api}` },
              },
            },
          });
        } else {
          responseChart = new Chart(rtCanvas.getContext("2d"), {
            ...
            data: {
              ...
              datasets: [
                {
                  label: "Response Time",
                  data: stats.responseTimesData?.durations || [],
                  borderColor: "#0066cc",
                  fill: false,
                },
              ],
            ...
            options: {
              plugins: {
                title: { display: true, text: "Response Times (Last 100)" },
              },
            },
          });
        }
        ...
        statusChart = new Chart(scCanvas.getContext("2d"), {
          ...
          data: {
            ...
            datasets: [
              {
                label: "Count",
                data: (stats.statusCodes || []).map((c) => c.count),
        ...
        typeChart = new Chart(tpCanvas.getContext("2d"), {
          ...
          data: {
            ...
            datasets: [
              {
                label: "Count",
                data: (stats.requestTypes || []).map((t) => t.count),
          ...
          options: {
            plugins: {
              title: { display: true, text: "Request Type Distribution" },
            },
          },
        });
          ...
          timeDistChart = new Chart(tdCanvas.getContext("2d"), {
            ...
            data: {
              ...
              datasets: [
                {
                  label: "Requests",
                  data: stats.timeDistribution.counts || [],
          ...
          sizeDistChart = new Chart(szCanvas.getContext("2d"), {
            ...
            data: {
              ...
              datasets: [
                {
                  label: "Requests",
                  data: stats.sizeDistribution.counts || [],
        ...
        if (canViewPerf && stats.avgTimings) {
          perfChart = new Chart(pfCanvas.getContext("2d"), {
              ...
              datasets: [
                {
                  label: "Avg (ms)",
                  data: [
                    stats.avgTimings.dns,
                    stats.avgTimings.tcp,
                    stats.avgTimings.ssl,
                    stats.avgTimings.ttfb,
                    stats.avgTimings.download,
                  ],
                  backgroundColor: [
                    "#3f51b5",
                    "#009688",
            ...
            options: {
              plugins: {
                title: { display: true, text: "Avg Performance Timings" },
              },
            },
          });
        }
  ...
  tabs.forEach((tab) => {
    ...
    btn.className =
      "analytics-tab-btn" + (tab.id === activeTab ? " active" : "");
    btn.onclick = () => {
      activeTab = tab.id;
      updateTabUI();
    };
  ...
  function updateTabUI() {
    tabBar
      ...
      .forEach((btn) => btn.classList.toggle("active", btn.id === activeTab));
    analysisSection.style.display =
      activeTab === "tab-analysis" ? "block" : "none";
    perfSection.style.display =
      activeTab === "tab-performance" ? "block" : "none";
    // Show visualizationsGroup only in analysis tab
    visualizationsGroup.style.display =
      activeTab === "tab-analysis" ? "" : "none";
    updateAnalysisChartUI();
    updatePerfChartUI();
  }
  function updateAnalysisChartUI() {
    ...
    analysisChartTabs.querySelectorAll("button").forEach((btn) => {
      ...
      btn.disabled =
        !visualizationCheckboxes[btn.id.replace("analysis-", "")]?.checked;
    });
    // If no chart is enabled, exit
    if (
      !activeAnalysisChart ||
      !visualizationCheckboxes[activeAnalysisChart]?.checked
    ) {
      return;
    }
    ...
    if (activeDiv) {
      ...
      if (activeAnalysisChart === "apiOverTime" && responseChart) {
        responseChart.destroy();
        responseChart = null;
      }
      if (activeAnalysisChart === "status" && statusChart) {
        statusChart.destroy();
        statusChart = null;
      }
      if (activeAnalysisChart === "type" && typeChart) {
        typeChart.destroy();
        typeChart = null;
      }
      if (activeAnalysisChart === "timeDist" && timeDistChart) {
        timeDistChart.destroy();
        timeDistChart = null;
      }
      if (activeAnalysisChart === "sizeDist" && sizeDistChart) {
        sizeDistChart.destroy();
        sizeDistChart = null;
      }
      ...
      pendingAnalyticsRequests[requestId] = (stats) => {
        if (!stats || stats.error) {
          activeDiv.innerHTML = `<div class='error'>${
            stats?.error || "No analytics data."
          }</div>`;
          return;
        }
        ...
        if (activeAnalysisChart === "apiOverTime") {
          const canvas =
            activeDiv.querySelector("canvas") ||
            document.createElement("canvas");
          if (!activeDiv.contains(canvas))
            (activeDiv.innerHTML = ""), activeDiv.appendChild(canvas);
      ...
      chrome.runtime.sendMessage({
        action:
          activeAnalysisChart === "apiOverTime"
    ...
    const activeBtn = analysisChartTabs.querySelector(
      `#analysis-${activeAnalysisChart}`
    );
  ...
  function updatePerfChartUI() {
    perfChartTabs
      .querySelectorAll("button")
      .forEach((btn) =>
        btn.classList.toggle("active", btn.id === "perf-" + activePerfChart)
      );
  ...
  function renderPerformanceChart(chartId, filters) {
    ...
    if (chartId === "waterfall") {
      ...
      if (window._waterfallChart) {
        window._waterfallChart.destroy();
        window._waterfallChart = null;
      }
      const requestId = generateRequestId();
      pendingAnalyticsRequests[requestId] = (res) => {
        ...
        if (
          res &&
          res.success &&
          Array.isArray(res.timings) &&
          res.timings.length > 0
        ) {
          // Prepare data for waterfall chart
          const labels = res.timings.map(
            (t) => t.name || t.url || t.initiatorType || ""
          );
          ...
          const dataset = [
            {
              ...
              data: res.timings.map((t) => ({
                x: t.startTime,
                y: labels.indexOf(t.name || t.url || t.initiatorType || ""),
                x2: t.startTime + t.duration,
              })),
          ...
          window._waterfallChart = new Chart(canvas.getContext("2d"), {
            ...
            options: {
              ...
              plugins: {
                title: {
                  display: true,
                  text: "Resource Waterfall (Start → End)",
                },
          ...
          waterfallDiv.innerHTML =
            '<div class="error">No waterfall data.</div>';
        }
      };
      chrome.runtime.sendMessage({
        action: "getResourceTimings",
        filters,
        requestId,
      });
      return;
    ...
    // --- Page Load Chart ---
    if (chartId === "pageload") {
      if (window._pageLoadChart) {
        window._pageLoadChart.destroy();
        window._pageLoadChart = null;
      }
      const requestId = generateRequestId();
      pendingAnalyticsRequests[requestId] = (res) => {
    ...
    if (chartId === "resource") {
      if (window._resourceBreakdownChart) {
        window._resourceBreakdownChart.destroy();
        window._resourceBreakdownChart = null;
      }
      const requestId = generateRequestId();
      pendingAnalyticsRequests[requestId] = (res) => {
        ...
        if (res && res.success && res.breakdown) {
          ...
          window._resourceBreakdownChart = new Chart(canvas.getContext("2d"), {
            ...
            options: {
              plugins: {
                title: { display: true, text: "Resource Type Breakdown" },
          ...
          resourceDiv.innerHTML =
            '<div class="error">No resource breakdown data.</div>';
        }
      };
      chrome.runtime.sendMessage({
        action: "getResourceBreakdown",
        filters,
        requestId,
      });
      return;
