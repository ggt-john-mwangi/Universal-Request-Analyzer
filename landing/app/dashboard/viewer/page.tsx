...
export default async function ViewerDashboard() {
  ...
  if (!user.roles.includes("Viewer")) return <div className="text-red-600">Not authorized</div>;

  // Fetch viewer dashboard data (read-only requests)
  const dashRes = await fetch(`/api/viewer`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  ...
  return (
    <main className="min-h-screen bg-gradient-to-r from-[#1F1A3D] via-[#4A00E0] to-[#00DFD8] p-8">
      <div className="max-w-5xl mx-auto bg-white/90 dark:bg-[#0a0a0a]/90 rounded-2xl shadow-xl p-8">
        <h1 className="text-2xl font-bold mb-6 text-[#4A00E0]">Viewer Dashboard</h1>
        <section>
          <h2 className="text-xl font-semibold mb-2">Recent Requests (Read-Only)</h2>
