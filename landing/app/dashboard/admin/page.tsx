...
export default async function AdminDashboard() {
  ...
  if (!user.roles.includes("Admin")) return <div className="text-red-600">Not authorized</div>;

  // Fetch admin dashboard data (user management, request stats, etc.)
  const dashRes = await fetch(`/api/admin`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  if (!dashRes.ok) return <div className="text-red-600">Failed to load dashboard</div>;
  const { users, requests } = await dashRes.json();
  ...
  return (
    <main className="min-h-screen bg-gradient-to-r from-[#1F1A3D] via-[#4A00E0] to-[#00DFD8] p-8">
      <div className="max-w-5xl mx-auto bg-white/90 dark:bg-[#0a0a0a]/90 rounded-2xl shadow-xl p-8">
        <h1 className="text-2xl font-bold mb-6 text-[#4A00E0]">Admin Dashboard</h1>
